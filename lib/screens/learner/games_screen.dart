...
class _GamesScreenState extends State<GamesScreen>{
  ...
  @override Widget build(BuildContext context)=>ListView(padding:const EdgeInsets.all(16),children:[
    ...
    SegmentedButton<int>(segments:const [ButtonSegment(value:0,icon:Icon(Icons.traffic),label:I18nText('Start Lights')),ButtonSegment(value:1,icon:Icon(Icons.timer),label:I18nText('Training Timer'))],selected:{tab},onSelectionChanged:(s)=>setState(()=>tab=s.first)),const SizedBox(height:14),
    if(tab==0)LightsPractice(profile:widget.profile),if(tab==1)TrainingTimer(profile:widget.profile,dog:widget.dog),
  ]);
}
...
class _LightsPracticeState extends State<LightsPractice>{
  ...
  Future<void> stop()async{if(!running||greenAt==null)return;final delta=DateTime.now().difference(greenAt!).inMicroseconds/1000000.0;for(final t in timers)t.cancel();timers.clear();setState((){running=false;result=delta;});final stats=await service.recordLightAttempt(uid:widget.profile.id,deltaSeconds:delta);if(delta>=0&&delta<.005){confetti.play();try{await audio.play(AssetSource('sounds/trophy_chime.wav'));}catch(_){}}else if(delta<0&&delta>-.005){try{await audio.play(AssetSource('sounds/aww_chime.wav'));}catch(_){}}if(mounted&&stats['rollingStreak']==3){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:I18nText('🔥 THREE ROLLING STARTS IN A ROW — Triple Broadside!')));}}
...
class TrainingTimer extends StatefulWidget{final AppUser profile;final DogProfile dog;const TrainingTimer({super.key,required this.profile,required this.dog});@override State<TrainingTimer> createState()=>_TrainingTimerState();}
class _TrainingTimerState extends State<TrainingTimer>{
  final audio=AudioPlayer();final service=FirestoreService();Timer? timer;int selected=2;int left=120;bool running=false;
  @override void dispose(){timer?.cancel();audio.dispose();super.dispose();}
  void start(){timer?.cancel();setState((){left=selected*60;running=true;});timer=Timer.periodic(const Duration(seconds:1),(t)async{if(left<=1){t.cancel();setState((){left=0;running=false;});await _alarm();if(mounted)_finishDialog();}else if(mounted)setState(()=>left--);});}
  Future<void> _alarm()async{HapticFeedback.mediumImpact();final file=switch(widget.profile.timerSound){'bell'=>'ship_bell.wav','cannon'=>'tiny_cannon.wav','none'=>'','parrot'=> 'parrot_squawk.wav',_=>'parrot_squawk.wav'};if(file.isNotEmpty){try{await audio.play(AssetSource('sounds/$file'));}catch(_){}}}
  Future<void> _finishDialog()async{String result='Good';String skill='General';final note=TextEditingController();final ok=await showDialog<bool>(context:context,builder:(ctx)=>StatefulBuilder(builder:(ctx,setLocal)=>AlertDialog(title:const I18nText('Time, matey! 🦜'),content:SizedBox(width:480,child:Column(mainAxisSize:MainAxisSize.min,children:[const I18nText('Finish on a good one and give your dog a break. Want to save the session to the diary?'),const SizedBox(height:10),DropdownButtonFormField<String>(initialValue:skill,items:['General',...courseModules.map((m)=>m.title)].map((e)=>DropdownMenuItem(value:e,child:I18nText(e))).toList(),onChanged:(v)=>setLocal(()=>skill=v??skill),decoration:const InputDecoration(label: I18nText('Skill'))),const SizedBox(height:8),DropdownButtonFormField<String>(initialValue:result,items:['Struggled','Getting there','Good','Nailed it!'].map((e)=>DropdownMenuItem(value:e,child:I18nText(e))).toList(),onChanged:(v)=>setLocal(()=>result=v??result),decoration:const InputDecoration(label: I18nText('How did it go?'))),const SizedBox(height:8),TextField(controller:note,decoration:const InputDecoration(label: I18nText('Optional note')))])),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const I18nText('Not this time')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const I18nText('SAVE TO DIARY'))])));if(ok==true)await service.addTrainingLog(uid:widget.profile.id,dogId:widget.dog.id,skill:skill,result:result,note:note.text,minutes:selected);note.dispose();}
  @override Widget build(BuildContext context)=>Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
    Card(child:Padding(padding:const EdgeInsets.all(18),child:Column(children:[I18nText('Short Training Timer',style:Theme.of(context).textTheme.titleLarge),const I18nText('Short, happy sessions are often better than drilling the same exercise.'),const SizedBox(height:14),I18nText('${left~/60}:${(left%60).toString().padLeft(2,'0')}',style:const TextStyle(fontSize:56,fontWeight:FontWeight.bold)),const SizedBox(height:12),if(!running)Wrap(spacing:8,children:[1,2,3,5,10].map((m)=>ChoiceChip(label:I18nText('$m min'),selected:selected==m,onSelected:(_)=>setState((){selected=m;left=m*60;}))).toList()),const SizedBox(height:12),SizedBox(width:260,child:FilledButton.icon(onPressed:running?(){timer?.cancel();setState(()=>running=false);}:start,icon:Icon(running?Icons.stop:Icons.play_arrow),label:I18nText(running?'STOP TIMER':'START TIMER'))),const SizedBox(height:8),I18nText('End sound: ${widget.profile.timerSound.toUpperCase()}')]))),
    const Card(child:Padding(padding:EdgeInsets.all(14),child:I18nText('Change the timer sound in Settings: Parrot Squawk, Ship’s Bell, Tiny Cannon or None.'))),
  ]);
}
