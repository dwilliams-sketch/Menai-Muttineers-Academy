...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> _reportPayment() async {
        ...
        builder: (ctx, setLocal) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const I18nText('One Doubloon is £5 and gives one dog 30 days of Academy access.'),
              const SizedBox(height: 8),
              TextField(
                controller: amount,
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: const InputDecoration(label: I18nText('Amount sent (£)')),
              ),
              const SizedBox(height: 8),
    ...
    if (ok == true && value != null && value > 0) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: I18nText('Payment notice sent to Admin.')));
      }
    }
    amount.dispose();
  }
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => AlertDialog(
            ...
            child: SingleChildScrollView(
              child: Column(children: [
                ...
                CheckboxListTile(
                  ...
                  value: estimated,
                  onChanged: (v) => setLocal(() => estimated = v ?? false),
                  title: const I18nText('Date is estimated'),
                ),
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const I18nText('Cancel')),
            FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const I18nText('Add dog')),
          ],
        ),
      ),
    );
    if (ok == true && name.text.trim().isNotEmpty) {
      await service.addDog(
        uid: widget.profile.id,
        name: name.text,
        breed: breed.text,
      ...
      builder: (context, academySnap) {
        ...
        return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
          ...
          builder: (context, linkSnap) {
            ...
            return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              stream: service.userStream(widget.profile.id),
              builder: (context, userSnap) {
                final liveProfile = userSnap.data?.exists == true ? AppUser.fromDoc(userSnap.data!) : widget.profile;
                final status = widget.dog.effectiveStatus(liveProfile);
                return ListView(
                  ...
                  children: [
                      ...
                      child: Padding(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                          Row(children: [
                            ...
                            Expanded(child: I18nText('Academy Doubloons', style: Theme.of(context).textTheme.titleLarge)),
                            I18nText(doubloonBalanceLabel(liveProfile.academyCredit), style: Theme.of(context).textTheme.headlineSmall),
                          ]),
                          const SizedBox(height: 8),
                          const I18nText('1 Doubloon = £5 = 30 days of Academy access for one dog.'),
                          const I18nText('Doubloons appear here as soon as Admin confirms or adjusts your balance.'),
                        ]),
                      ),
                    ),
                    Card(
                        ...
                        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                          ...
                          if (status == 'active') ...[
                            ...
                            if (widget.dog.pauseRequested) ...[
                              const I18nText('⚓ Pause scheduled. No new Doubloon will be used when this paid voyage ends.'),
                              OutlinedButton(onPressed: () => service.cancelDogPause(widget.dog.id), child: const I18nText('KEEP ADVENTURE GOING')),
                            ] else
                              OutlinedButton.icon(
                                onPressed: () => service.requestDogPause(uid: widget.profile.id, dogId: widget.dog.id),
                                icon: const Icon(Icons.pause_circle_outline),
                                label: const I18nText('PAUSE AT END OF CURRENT VOYAGE'),
                              ),
                          ],
                          if (status == 'paused' || status == 'renewal_due' || status == 'awaiting') ...[
                            ...
                            FilledButton.icon(
                              onPressed: () async {
                                ...
                                if (mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: I18nText('Restart request sent to Admin.')));
                                }
                              },
                              icon: const Icon(Icons.play_circle),
                              label: const I18nText('RESTART ADVENTURE'),
                            ),
                            const I18nText('Admin will use 1 available Doubloon to start another 30 days of access.'),
                          ],
                        ]),
                      ),
                    ),
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(16),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                          I18nText('Add Doubloons', style: Theme.of(context).textTheme.titleLarge),
                          ...
                          FilledButton.icon(
                            ...
                            label: const I18nText('I’VE MADE A PAYMENT'),
                          ),
                        ]),
                      ),
                    ),
                    ...
                    I18nText('Doubloon History', style: Theme.of(context).textTheme.titleLarge),
                    StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                      stream: service.ledgerForUser(widget.profile.id),
                      builder: (context, snap) {
                        final docs = [...(snap.data?.docs ?? [])];
                        docs.sort((a, b) => ((b.data()['createdAt'] as Timestamp?)?.millisecondsSinceEpoch ?? 0)
                            .compareTo((a.data()['createdAt'] as Timestamp?)?.millisecondsSinceEpoch ?? 0));
                        if (docs.isEmpty) {
                          return const Card(child: Padding(padding: EdgeInsets.all(16), child: I18nText('No account activity recorded yet.')));
                        }
                        return Column(
                          children: docs.take(30).map((d) {
                            final m = d.data();
                            final amount = (m['amount'] as num?)?.toDouble() ?? 0;
                            final doubloons = poundsToDoubloons(amount.abs());
                            final amountLabel = '${amount >= 0 ? '+' : '-'}${formatDoubloonNumber(doubloons)} ${doubloons == 1 ? 'Doubloon' : 'Doubloons'}';
                            return Card(
                              child: ListTile(
                                leading: Icon(amount >= 0 ? Icons.add_circle : Icons.remove_circle),
                                title: I18nText((m['description'] ?? 'Academy account').toString()),
                                trailing: I18nText(amountLabel),
                              ),
                            );
                          }).toList(),
                        );
